...
@comment_router.get("/api/post/comment",tags=['comment'])
def read_api(db: session = Depends(CommentService.get_db)):
    result = CommentService(db).get_comments()
    return JSONResponse(content=jsonable_encoder(result), status_code=200)

@comment_router.post("/api/user/me/post/comment",tags=['comment'],status_code=201,response_model=dict)
...
@comment_router.delete('/api/user/post/comment/{id}',tags=['comment'])
...
@comment_router.get("/api/user/comment/{status}",tags=['comment'])
...
@comment_router.put('/api/user/comment/{id}',tags=['comment'])
